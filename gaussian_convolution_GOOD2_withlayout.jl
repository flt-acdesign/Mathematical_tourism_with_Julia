...
# ╔═╡ ba8c3ecb-11eb-4b66-99b7-0a0b9540b796
using Images, PlutoUI

# ╔═╡ da7bf964-97d2-47e8-91b8-31dee680404b
using Plots

# ╔═╡ c2b3dcc4-df75-46a5-a224-a89224e71d06
function gaussian_kernel(σ)
    k = 2 * ceil(Int, 3.0 * σ) + 1
    kernel = [exp(-(i^2 + j^2) / (2 * σ^2)) for i in -k:1:k, j in -k:1:k]
    kernel /= sum(kernel)
    return kernel
end

# ╔═╡ fe2964ad-6614-427f-a553-ba55a9f02694
function gaussian_convolution(image, σ)
    kernel = gaussian_kernel(σ)
    return imfilter(image, kernel, "circular")
end

# ╔═╡ 9b9a3349-55a8-4bfe-9227-6f718c5af77c
# Example usage
img = rand(100, 100);  # Replace this with your image data

# ╔═╡ 7cc9b961-ad9b-49fd-9194-c4dac11dc855
@bind σ Slider(1:10)

# ╔═╡ d248f9ad-ae12-47df-94b1-aa2057468ce1
heatmap(gaussian_kernel(σ))

# ╔═╡ f4683f97-85e2-44f4-a746-d35a30eaa4d0
smoothed_img = gaussian_convolution(img, σ);

# ╔═╡ e39dfb00-3a2a-4efd-a60a-f34259753bea
...
manifest_format = "2.0"
project_hash = "439ba413fc84e4197fb8adc646955bf7a3839c94"
